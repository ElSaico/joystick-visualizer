...
(function(sd) {
    ...
    this.make_table = function(dict) {
        const res = $t.element('div');
        let table = $t.element('table', {'class': "ui-table"}, res);
        for (const i in dict) {
            let name = String(i);
            if (name[0] === '-' && name[1] === '-') {
    ...
    this.make_list = function(container, val, list) {
        for (const i in list) {
            let e = list[i], ind = i, value = list[i];
            if (typeof e == 'object') { ind = e[0]; value = e[1]; }
            const o = $t.element('div', {'class': 'small-list-item', value: ind}, container, value);
            if (ind === val) $t.raise(o, 'tealgui-item-selected', o, true, true);
            $t.bind(o, 'mouseup', function(ev) { 
                    $t.raise(this, 'tealgui-item-selected', this, true, true); });
        }
    }

    this.make_mask = function(container, val, list) {
        let selected = 0;
        $t.bind(container, 'tealgui-item-selected', function(ev) {
            ev.stopPropagation();
            const o = ev.detail;
            const s = o.getAttribute('data-selected') === '1';
        ...
        for (const i in list) {
            ...
            if (ind & val) {
                o.initial = true;
                $t.raise(o, 'tealgui-item-selected', o, true, true);
            }
            $t.bind(o, 'mouseup', function(ev) { 
                    $t.raise(this, 'tealgui-item-selected', this, true, true); });
        }
    ...
    this.make_tree = function(container, val, tree, expand) {
        function on_toggle(items) {
            const s = parseInt(this.parentNode.getAttribute('data-expanded'));
            if (s >= 2) return;
            if (s === -1) {
                let m;
                if (items !== undefined) {
                ...
                if (typeof items === 'function') {
                    ...
                    const dd = this;
                    $t.bind(this, 'tealgui-item-update', function() {
                        const deferred = $t.deferred();
                        items(dd, deferred);
                        $t.empty(m);
                        deferred.promise().done(function(tree) { fill_tree(tree, m); });
                    });
                    $t.raise(this, 'tealgui-item-update', this, true, true);
                }
            }
        ...
        function fill_tree(tree, d) {
            ...
            for (const i in tree) {
                ...
                const compl = typeof e[1] == 'object' && e[1].nodeName === undefined;
                if (compl) value = e[1].label;
                const c = $t.element('div', {'class': 'small-tree-item'}, d);
                const exp = $t.element('div', {'class': 'small-tree-item-exp'}, c);
                ...
                const o = $t.element('div', {'class': 'small-list-item', value: ind}, c, value);
                if (compl) {
                    const set = {};
                    for (const k in e[1]) set[`data-${k}`] = e[1][k];
                    $t.set(o, set);
                }
                if (ind === val) {
                    o.initial = true;
                ...
                $t.bind(o, 'mouseup', function(ev) { $t.raise(this, 'tealgui-item-selected', this, true, true); });
            }
        }
        ...
        if (!was_set && val !== undefined) {
            $t.raise(container, 'tealgui-item-text', '↻', true, true);
        }
    }

    const controller_factory = {
        'number': function (val) {
            const el = $t.element('input', {type: 'number', step: "any", 'class': 'small-input dialog-input'});
            el.value = val;
            return el;
        },
        'area': function (val) {
            ...
            for (const i in t) if (c < t[i].length) c = t[i].length;
            const el = $t.element('textarea', {rows: r, cols: c, 'class': 'small-input dialog-input'});
            el.value = val;
            return el;
        },
        'text': function (val) {
            const el = $t.element('input', {type: 'text', 'class': 'small-input dialog-input'});
            el.value = val;
            return el;
        },
        'password': function (val) {
            const el = $t.element('input', {type: 'password', 'class': 'small-input dialog-input'});
            el.value = val;
            return el;
        },
        'datetime': function (val) {
            const el = $t.element('input', {type: 'datetime-local', 'class': 'small-input dialog-input'});
            el.style.width = "16em";
            const d = new Date(d - d.getTimezoneOffset() * 60 * 1000);
            el.value = d.toISOString().split('Z')[0];
            return el;
        },
        'ipv4': function (val) {
            const el = $t.element('input', {
                type: 'text', 'data-pattern': 'ip',
                pattern: '((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)',
                'class': 'small-input dialog-input'
            });
            const ip = parseInt(val);
            el.value = `${(ip >> 24) & 0xFF}.${(ip >> 16) & 0xFF}.${(ip >> 8) & 0xFF}.${ip & 0xFF}`;
            return el;
        },
        'color': function (val) {
            const el = $t.element('input', {type: 'color', 'class': 'small-input dialog-input'});
            el.value = val;
            return el;
        },
        'bool': function (val) {
            const el = $t.element('input', {type: 'checkbox', 'class': 'small-input-checkbox dialog-input'});
            el.checked = val ? 1 : 0;
            return el;
        },
        'list': function (val, list) {
            const dd = $t.gui.dropdown();
            $t.gui.make_list(dd.d, val, list);
            return dd.div;
        },
        'mask': function (val, list) {
            const el = $t.element('div', {'class': 'dialog-input small-input-mask', value: val});
        ...
        'file': function (name) {
            return $t.element('input', {type: 'file', name: name, 'class': 'small-input dialog-input'});
        }
    };
    ...
    this.make_input = function(type, val, params, ro) {
        const f = controller_factory[type];
        const el = f ? f(val, params) : '';
        if (ro) {
    ...
    this.make_table_inputs = function(dict) {
        ...
        for (const i in dict) {
            ...
            if (label[0] === '[') {
                ...
                const cc = label.substring(1, cb).split(';');
                for (const c in cc) {
                    const m = cc[c].split(':');
                    conditions[m[0]] = m[1];
                }
            ...
            if (type[0] === '[') {
                ...
                const rs = $t.element('span', {});
                for (const l in types) {
                    type = types[l];
        ...
        const table = $t.gui.make_table(dd);
        const cs = table.getElementsByClassName('dialog-input');
        for (const i in cons) controll_apply_conditions(cs, i, cons[i]);
        return table;
    }

    this.button_group = function(sel, index) {
        const buttons = sel.children;
    ...
    this.dropdown = function(pardiv) {
        const div = $t.element('div', {'class': 'small-input-tree'});
        const el = $t.element('span', {'class': 'small-input-tree dialog-input'}, div);
        const d = $t.element('div', {'class': 'small-input-dropdown'}, pardiv ? pardiv : div);
        let visible = false;
        let selected = undefined;
        const hideout = function (ev) {
            if (!visible) return;
        ...
        $t.bind(d, 'tealgui-item-selected', function(ev) {
            ev.stopPropagation();
            const o = ev.detail;
            if (o.getAttribute('data-selectable') === '0') return;
            if (selected) $t.set(selected, { 'data-selected': 0 });
        ...
        $t.bind(d, 'tealgui-item-text', function(ev) {
            ev.stopPropagation();
            el.innerHTML = ev.detail;
        });
        $t.bind(div, 'tealgui-item-text', function(ev) {
            ev.stopPropagation();
            el.innerHTML = ev.detail;
        });
    ...
    this.dialog = function() {
        return $t.element('div', {'class': 'ui-inner'},
    ...
    this.dialog_buttons = function(title, buttons, noxbutton) {
        const b = $t.element('div', { style: 'margin-top: 10px; margin-bottom: 10px;' }, d);
        const d = $t.gui.dialog();
        if (!noxbutton) {
            const cb = $t.element('button', {'class': 'dialog-close-button'});
            const img = $t.svg.svg('svg', {version: '1.1', viewBox: '0 0 10 10', width: 16, height: 16}, cb);
        ...
        $t.element('br', {}, d);
        const btns = $t.element('div', {'class': 'dialog-button-pane'}, d);
        for (const i in buttons) {
            if (i !== 'X')
                (function(i, callback) {
                    const bb = $t.element('button', {'class': 'small-text-button'}, btns);
    ...
    this.message = function(text) {
        const d = $t.gui.dialog_buttons(undefined, {
            'OK': function (d) {
    ...
    this.message_ask = function(text, buttext, callback) {
        ...
        const d = $t.gui.dialog_buttons(undefined, bb);
        $t.clas(d[0], null, "ui-messagebox");
        $t.clas(d[1], null, "ui-message-div");
        let t = text;
        if (text.nodeName === undefined) {
    ...
    this.folder = function(caption, content, place, opened) {
        const div = $t.element('div', {}, place);
        const c = $t.element('div', {style: 'background: lightgray;'}, div);
        const b = $t.element('button', {style: 'padding: 2px; background: none; border: none; text-align: left'}, c);
        const ar = $t.element('b', {style: 'margin-right: 4px'}, b, opened ? '▼' : '►');
        $t.element('b', {}, b, caption);
        const n = $t.element('div', {style: 'padding: 4px'}, div, content);
